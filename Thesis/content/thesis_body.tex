...
Testing and evaluation has been done by myself by referring back to the requirements I stated earlier in chapter four.\\
...
In the following chapter I will talk about each of these requirements and whether and in howfar my prototype can fulfill them.
...
However, the JSON version for this same agent, comes in at exactly 200 lines as you can see in the appendix. \ref{}REFTHISLATER \url{https://github.com/arne-z/BachelorThesis/commit/d61614158d41d72bb72f28f941718a707d72a5e0}

This demonstrates, that the DSL code for an agent is significantly shorter than the JSON version.
The benefit of thia is that less code causes better maintainability and faster and easier code reviews. 
...
Below you can see an example of the CircleCI job configuration used with the prototype in our bachelor project.
\footnote{The full configuration can be found here.\\
...
The above configuration starts a docker image with java and node installed - java is required for the DSL compiler, JavaScript for the import script - and checks out the most recent version of our project from source control.
...
It then saves our project's google authentication key in a JSON file and runs the importAgent script that will send the JSONs generated by the compiler to the Dialogflow API.

This setup allows a "hands free" approach to developing for Dialogflow, where the developer has to only change the DSL code and once that is pushed to master on git, the Dialogflow agent is updated automatically.
